...
describe('SwitchSlot', () => {
  it('check default Switch Slot values', () => {
    ...
    expect(defaultData.headers[4].text).to.equal('Quantity of Ports');
    expect(defaultData.headers[5].text).to.equal('Band');
    expect(defaultData.headers[6].type).to.equal('select');
  });
});
